...
<h3 align="center" style="color:#FFFFFF"> Delete Course</h3>
</div>
...
<form  class="form1" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<label>Course Code </label>	
					<select name="ccode" >
						...
						<?php
						$result = mysqli_query($con, "SELECT CourseCode FROM tblcourses ORDER BY CourseCode ASC ");
						while ($row = mysqli_fetch_array($result)) {
...
$sql = "DELETE FROM tblcourses WHERE CourseCode='$ccode1'";
$result = mysqli_query($con, $sql);	

	// mysqli_query($con, "DELETE FROM tblcourses WHERE CourseCode='".$row['CourseCode']."'")
